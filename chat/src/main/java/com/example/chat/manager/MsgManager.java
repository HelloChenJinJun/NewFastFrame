...
public class MsgManager {
        ...
        private MsgManager() {
                mPushManager = new BmobPushManager<>(BaseApplication.getInstance());
        }
...
//                                                                推送失败后也要把消息保存到服务器上，方便在接收方那边从服务器上拉去数据
        ...
        private void saveMessageToService(ChatMessage msg) {
                msg.save(BaseApplication.getInstance(), new SaveListener() {
                                @Override
                                public void onSuccess() {
...
//                                        alert = message.getBelongUserName() + "已同意添加你为好友";
        ...
        private void getUserById(String targetId, FindListener<User> findListener) {
                ...
                query.findObjects(BaseApplication.getInstance(), findListener);
        }
                ...
                queryMsg(isBelongId, id, createTime, new FindListener<ChatMessage>() {
                                ...
                                public void onSuccess(List<ChatMessage> list) {
                                        if (list != null && list.size() > 0) {
                                                ...
                                                chatMessage.update(BaseApplication.getInstance(), new UpdateListener() {
                                                                @Override
                                                                public void onSuccess() {
...
//                        群消息发送
        ...
        private void saveAndUploadGroupChatMessage(GroupChatMessage message, SaveListener listener) {
                ...
                message.save(BaseApplication.getInstance(), listener);
        }
...
//                                query.addWhereEqualTo("groupId", groupId);
                                query.findObjects(BaseApplication.getInstance(), new FindCallback() {
                                        @Override
...
//                recentMsg.setNick(message.getGroupName());
        ...
        private void uploadSendMessage(GroupChatMessage groupChatMessage) {
                groupChatMessage.save(BaseApplication.getInstance(), new SaveListener() {
                                @Override
                                public void onSuccess() {
        ...
        private void uploadSendMessage(ChatMessage message) {
                message.save(BaseApplication.getInstance(), new SaveListener() {
                                @Override
                                public void onSuccess() {
...
//                MB单位
                ...
                bmobFile.uploadblock(BaseApplication.getInstance(), new UploadFileListener() {
                        ...
                        public void onSuccess() {
                                LogUtil.e("上传文件成功");
                                String fileUrl = bmobFile.getFileUrl(BaseApplication.getInstance());
...
//                                        bmobQuery.addWhereEqualTo("createdTime", time);
                                        ...
                                        bmobQuery.findObjects(BaseApplication.getInstance(), new FindListener<GroupTableMessage>() {
                                                @Override
                                                public void onSuccess(List<GroupTableMessage> list) {
...
//                                                                设置未读取状态，方便定时拉取到
                                                                ...
                                                                groupTableMessage.update(BaseApplication.getInstance(), new UpdateListener() {
                                                                        @Override
                                                                        public void onSuccess() {
...
//                                                                                这里先上传再推送，因为对方可能接收到推送信息的时候无法即时获取到服务器上的群结构消息,也就查询信息失败
        ...
        private void uploadChatTableMessage(final GroupTableMessage groupTableMessage, SaveListener listener) {
                ...
                new BmobObject().insertBatch(BaseApplication.getInstance(), groupTableMessageList, listener);
        }
...
//                这里先不通过组播推送消息，因为在对方用户还没有接受建群的消息的时候(也就意味着没订阅该群消息)，发送欢迎消息会接收不到的
        ...
        public void updateGroupTableMessageReaded(GroupTableMessage message, UpdateListener listener) {
                message.update(BaseApplication.getInstance(), listener);
        }
...
//                message.setGroupNumber(CommonUtils.string2list(JsonUtil.getString(jsonObject, Constant.GROUP_NUMBER)));
        ...
        public void sendShareMessage(final SharedMessage shareMessage, final AddShareMessageCallBack listener) {
                ...
                shareMessage.save(BaseApplication.getInstance(), new SaveListener() {
                        @Override
                        public void onSuccess() {
                                ...
                                query.findObjects(BaseApplication.getInstance(), new FindListener<SharedMessage>() {
                                        @Override
        ...
        public void addLiker(final String id, final DealMessageCallBack dealMessageCallBack) {
                ...
                sharedMessage.update(BaseApplication.getInstance(), new UpdateListener() {
                        @Override
                        public void onSuccess() {
        ...
        public void deleteShareMessage(final String id, final DealMessageCallBack dealMessageCallBack) {
                ...
                if (sharedMessage.getMsgType().equals(Constant.MSG_TYPE_SHARE_MESSAGE_IMAGE) || sharedMessage.getMsgType().equals(Constant.MSG_TYPE_SHARE_MESSAGE_VIDEO)) {
                        BmobFile.deleteBatch(BaseApplication.getInstance(), sharedMessage.getImageList().toArray(new String[]{}), new DeleteBatchListener() {
                                @Override
                                public void done(String[] strings, BmobException e) {
                                        ...
                                        if (e != null) {
                                                ...
                                                LogUtil.e("全部文件删除成功");
                                                sharedMessage.delete(BaseApplication.getInstance(), new DeleteListener() {
                                                        @Override
                                                        public void onSuccess() {
...
//        }
        ...
        public void updateUserInstallationId(UpdateListener listener) {
                User currentUser = UserManager.getInstance().getCurrentUser();
                currentUser.setInstallId(CustomInstallation.getInstallationId(BaseApplication.getInstance()));
                currentUser.update(BaseApplication.getInstance(), listener);
        }
                ...
                findReadTag(conversationId, createTime, new FindListener<ChatMessage>() {
                        ...
                        public void onSuccess(List<ChatMessage> list) {
                                if (list != null && list.size() > 0) {
                                        ...
                                        message.setReadStatus(Constant.READ_STATUS_READED);
                                        message.update(BaseApplication.getInstance(), new UpdateListener() {
                                                @Override
                                                public void onSuccess() {
...
//                上面是分享的链接部分
                ...
                if (imageList != null && imageList.size() > 0) {
                        ...
                        BmobFile.uploadBatch(BaseApplication.getInstance(), urlList.toArray(new String[]{}), new UploadBatchListener() {
                                @Override
        ...
        public void updateUserAvatar(final String uid, final String avatar, final DealUserInfoCallBack dealUserInfoCallBack) {
                ...
                user.update(BaseApplication.getInstance(), new UpdateListener() {
                        @Override
                        public void onSuccess() {
        ...
        public void getAllDefaultAvatarFromServer(final FindListener<String> findListener) {
                ...
                bmobQuery.findObjects(BaseApplication.getInstance(), new FindCallback() {
                        @Override
                        public void onSuccess(JSONArray jsonArray) {
        ...
        public void queryAllGroupNumber(String groupId, FindListener<User> listener) {
                ...
                query.findObjects(BaseApplication.getInstance(), listener);
        }
        ...
        private void realUpdateGroupMessage(final String groupId, String name, final String content, final UpdateListener listener) {
                ...
                if (name.equals("groupNick")) {
                        ...
                        copy.update(BaseApplication.getInstance(), new UpdateListener() {
                                @Override
                                public void onSuccess() {
...
//                                                groupChatMessage.setReadStatus(Constant.RECEIVE_UNREAD);
                                ...
                                if (updateList.size() > 0) {
                                        new BmobObject().updateBatch(BaseApplication.getInstance(), updateList, new UpdateListener() {
                                                @Override
                                                public void onSuccess() {
...
//
//                query.findObjects(BaseApplication.getInstance(), new FindListener<GroupChatMessage>() {
//                        @Override
