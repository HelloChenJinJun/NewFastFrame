...
//        private ViewGroup.LayoutParams matchLayoutParams;
    ...
    private void initView() {
        ...
        if (mImageItemList.size() == 1) {
            ...
            CommonLogger.e("现在可见性为:"+getVisibility());
            for (int i = 0; i < getChildCount(); i++) {
                LinearLayout child= (LinearLayout) getChildAt(i);
                CommonLogger.e(i+"现在可见性为:"+child.getVisibility()
                +"item的个数"+ child.getChildCount()
...
//                这里获取控件的宽度，用户设置子控件的布局
    ...
    private boolean isEqual(List<String> list, List<String> mImageItemList) {
        if (mImageItemList != null && mImageItemList.size() == list.size()
                ) {
            int size = mImageItemList.size();
            for (int i = 0; i < size; i++) {
                if (!list.get(i).equals(mImageItemList.get(i))) {
                    return false;
                }
            }
            return true;
