...
public class NewsInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (request.url().toString().equals(NewsUtil.CARD_LOGIN_URL)) {
            ...
            if (cookie != null) {
                BaseApplication.getAppComponent()
                        .getSharedPreferences().edit().putString(NewsUtil.CARD_LOGIN_COOKIE,cookie)
                        .apply();
            }
            return response;
        }
//
        ...
        if (request.url().toString().startsWith(NewsUtil.CARD_POST_LOGIN_URL)) {
            Request newRequest;
            ...
            if (cookie != null) {
                BaseApplication.getAppComponent()
                        .getSharedPreferences().edit().putString(NewsUtil.CARD_POST_LOGIN_COOKIE,cookie)
                        .apply();
            }
            return response;
        }
        ...
        if (request.url().toString().equals(NewsUtil.CARD_PAGE_INFO_URL)||
           ...
           return chain.proceed(request.newBuilder().method(request.method(),request.body()).url(request.url())
                    .header("Cookie",getCardLoginCookie())
                    .build());
        }
    ...
    private String getCardLoginCookie() {
        return "_ga=GA1.3.1067555487.1498354603;"+BaseApplication.getAppComponent().getSharedPreferences().getString(NewsUtil.CARD_LOGIN_COOKIE, null)
                +";"+BaseApplication.getAppComponent().getSharedPreferences().getString(NewsUtil.CARD_LOGIN_COOKIE,null);
    }
}
